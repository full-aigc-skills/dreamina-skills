...
当用户需要 **在明确输入、预算和交付约束后执行生成或写入操作** 时加载本技能。先从请求中提取目标、输入、约束、交付格式和验收标准；描述摘要为："Use when an agent or script must save or run a Dreamina Canvas image node (t2i or i2i), compose ordered node: and res: references, separate generation edits from sparse metadata edits, and hand off paid execution to dreamina-canvas-quote-and-run."。
